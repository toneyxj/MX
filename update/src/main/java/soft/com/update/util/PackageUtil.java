...
//                Log.d("app","packageName:" + packageName);
        ...
        try {
            ...
            Log.i("app install ",e.getMessage());
            // process is still running, kill it.
            installCall.installed(-1,"安装失败");
            process.destroy();
        }
    ...
    public void reInstall(String apkPath,final InstallCall installCall){
        ...
        try {
            ...
            if ("".equals(result)){
                ...
                if (result.contains("Success")){
                    APPLog.d("app","update 覆盖安装成功：" + result);
                    // TODO: 2016/10/18 获取apk包名
                    installCall.installed(SUCCESS_CODE,apkPath);
                }else{
                    installCall.installed(-1,"覆盖安装失败result not endwith success" + result);
            ...
            APPLog.d("update","IOExceptio:" + e.getMessage());
            e.printStackTrace();
        } catch (Exception e) {
            APPLog.d("update","Exception:" + e.getMessage());
            e.printStackTrace();
        } finally {
            APPLog.e("update","finally");
            try {
                if (errIs != null) {
                    errIs.close();
                }
