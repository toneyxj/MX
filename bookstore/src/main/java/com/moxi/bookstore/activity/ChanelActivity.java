...
//            courent_page_tv.setText(courrentPage + "/");
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.skip_page:
                if (pageCount > 0)
                    PageSkipActivity.startPageSkip(this, courrentPage-1, data.size()/4);
                break;
            case R.id.last_page:
                goLastPage();
                break;
    ...
    private void initAdapter(){
        ...
        int dataPage=data.size()/4;
        dataPage+=data.size()%4==0?0:1;
        if (courrentPage>dataPage){
            courrentPage=dataPage;
        }
        pageData.clear();
        for (int i = (courrentPage - 1) * 4; i < (courrentPage * 4 > data.size() ? data.size() : courrentPage * 4); i++) {
