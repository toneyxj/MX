 ...
 */
public class NowSubjectDefautView extends LinearLayout implements RequestCallBack {
    ...
    public NowSubjectDefautView(Context context) {
        super(context);
        init(context);
    }

    public NowSubjectDefautView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }

    public NowSubjectDefautView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void postData() {
        if (UserInformation.getInstance().getID() != -1 && UserInformation.getInstance().getUserInformation() != null) {
            ...
            new HttpPostRequest(context, this, values, "3011", Connector.getInstance().getQuestion, false).execute();
        } else {
            if (callBack != null) {
    ...
    public void onSuccess(String result, String code) {
        ...
        Log.d("TAG",result);
        NowSubjectModel edm = GsonTools.getPerson(result, NowSubjectModel.class);
        if (edm != null && edm.getCode() == 0) {
            ACache.get(context).put(Connector.getInstance().getQuestion, result);
            if (callBack != null) {
