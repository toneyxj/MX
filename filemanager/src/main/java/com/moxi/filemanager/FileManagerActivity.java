...
//        mFragmentTransaction.commitAllowingStateLoss();
    ...
    private void dbackFragment(String path) {
        try {
            ...
            if (parentPath.equals("/mnt/sdcard") || parentPath.equals("/mnt/extsd")) {
                if (sonFragments.size() == 0) {
                    mFragmentTransaction.commitAllowingStateLoss();
                    addFragment(currentFragment);
                    return;
...
//        if (sonFragments.size() == 0 || show_title.getText().toString().equals(getString(R.string.souye))) {
        if (path.equals("/mnt/sdcard") || path.equals("/mnt/extsd")||sonFragments.size()==0) {
            this.finish();
        } else {
            dbackFragment(path);
        }
