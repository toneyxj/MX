...
import com.mx.home.model.MXUpdateMsg;
import com.mx.home.utils.UnzipFromFile;
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case 200:
                ...
                try {
                    share.setCache("moxi_app_version", updateMsg.getResult().getVersion());
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.img_update_from_net:
                String versionNo;
                if (share.getString("moxi_app_version").equals("")) {
                    versionNo = "MXB970S_C1_V1.1.0";
                } else {
                    versionNo = share.getString("moxi_app_version");
                }
                OkHttpUtils.getInstance().post().url(Constant.CHECK_VERSION).addParams("version", versionNo).build().execute(new StringCallback() {
                    @Override
                    ...
                    public void onResponse(String response, int id) {
                        try {
                            HomeActivity.wakeLock.acquire();
                            updateMsg = GsonTools.getPerson(response, MXUpdateMsg.class);
